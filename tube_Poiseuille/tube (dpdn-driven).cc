...
{	
	...
	build_grid();
	setup_system();
	seed_particles({2, 2});//устанавливаем частицы в ячейки и передаем им скорости с вершин
	...
	//на каждый шаг времени корректируем скорость, двиигаем частицы и выводим результаты
	for (; time<=15; time+=time_step, ++timestep_number) {		//ограничение по времени
