...
void pfem2Solver::seed_particles_into_cell (const typename DoFHandler<2>::cell_iterator &cell)
{
	...
	for(unsigned int i = 0; i < quantities[0]; ++i){	//цикл по количеству частиц по х
		...
		//вводим новую частицу и размещаем её в точке ([пред. частица] + 0,5) * ["шаг" между частицами]
			...
			//для каждой частицы смотрим вершины
			for (unsigned int vertex=0; vertex<GeometryInfo<2>::vertices_per_cell; ++vertex){
				shapeValue = fe.shape_value(vertex, particle->get_reference_location());//извлекаем функцию формы для конкретной вершины

				//устанавливаем скорости для частицы
				particle->set_velocity_component(particle->get_velocity_component(0) + shapeValue * solutionVx(cell->vertex_dof_index(vertex,0)), 0);//по х
				particle->set_velocity_component(particle->get_velocity_component(1) + shapeValue * solutionVy(cell->vertex_dof_index(vertex,0)), 1);//по у
			}//vertex
		}
	}
}
...
void pfem2Solver::seed_particles(const std::vector < unsigned int > & quantities)
{
	...
	if(quantities.size() < 2){ return; }	//количество частиц осей < 2, выходим из функции
	
	this->quantities = quantities; //устанавливаем вектор на то, что передали (откуда функция вызывается?? У текущей фигуры?????)
	
	typename DoFHandler<2>::cell_iterator cell = dof_handlerVx.begin(tria.n_levels()-1), endc = dof_handlerVx.end(tria.n_levels()-1);// итератор начала и концас скорости по х
	for (; cell != endc; ++cell) {
		seed_particles_into_cell(cell);	//"посев" частиц
	}
...
void pfem2Solver::correct_particles_velocities()
{
	...
	typename DoFHandler<2>::cell_iterator cell = dof_handlerVx.begin(tria.n_levels()-1), endc = dof_handlerVx.end(tria.n_levels()-1);	//итератор начало - конец (ст. свободы)
	for (; cell != endc; ++cell) {
...
void pfem2Solver::move_particles() //перенос частиц
{
	...
	for (int np_m = 0; np_m < PARTICLES_MOVEMENT_STEPS; ++np_m) {	//движение частиц
		...
		for (; cell != endc; ++cell) {	//цикл по ячейкам
			for( auto particleIndex = particle_handler.particles_in_cell_begin(cell); 
		                                   particleIndex != particle_handler.particles_in_cell_end(cell); ++particleIndex ) {	//частицы в каждой ячейке

				vel_in_part = Tensor<1,2> ({0,0});	//обнуляем тензор
				
				for (unsigned int vertex=0; vertex<GeometryInfo<2>::vertices_per_cell; ++vertex){	//цикл по вершинам вокруг частицы

					shapeValue = fe.shape_value(vertex, (*particleIndex).second->get_reference_location());	//функция формы конкретной вершины
					//задаем компоненты тензора как сумма функций формы на скорость в вершине
				...
				//получаем расстояние в тензоре
				vel_in_part[0] *= min_time_step;
				vel_in_part[1] *= min_time_step;
				

				//устанавливаем новое положение частицы как старое положение плюс полученное расстояние
		...
		//перенумеруем частицы в соответствии с текущим состоянием
	...
	for (; cell != endc; ++cell) {	//цикл по ячейкам
		...
		for (unsigned int vertex=0; vertex<GeometryInfo<2>::vertices_per_cell; ++vertex){	//цикл по вершинам
			...
			//	node_velocityY[cell->vertex_dof_index(vertex,0)] += shapeValue * (*particleIndex).second->get_velocity_component(1);
				...
				node_velocityX[cell->vertex_dof_index(vertex,0)] += 1.0 / pow(distance,2)//1.0 / (distance + (13.0 / hx_step)) 
				* (*particleIndex).second->get_velocity_component(0);
				node_velocityY[cell->vertex_dof_index(vertex,0)] += 1.0 / pow(distance,2)//1.0 / (distance + (4.0 / hy_step))
				* (*particleIndex).second->get_velocity_component(1);
			...
			//	node_weights[cell->vertex_dof_index(vertex,0)] += 1 / (distance+1);	
				...
				node_weightsX[cell->vertex_dof_index(vertex,0)] += 1.0 / pow(distance,2);//1.0 / (distance+(13.0 / hx_step));
				node_weightsY[cell->vertex_dof_index(vertex,0)] += 1.0 / pow(distance,2);//1.0 / (distance+(4.0 / hy_step));
		...
		for (unsigned int vertex=0; vertex<GeometryInfo<2>::vertices_per_cell; ++vertex){	//цикл по вершинам
			...
			for (auto particleIndex = particle_handler.particles_in_cell_begin(cell); 
				...
				{	
					sum += j;
					for (int s = 0; s <= j; s++)
		 				b[sum + s] = pow(mapping.transform_unit_to_real_cell(cell, (*particleIndex).second->get_reference_location())(0)-
						 cell->center()(0), j - s) *
						  pow(mapping.transform_unit_to_real_cell(cell, (*particleIndex).second->get_reference_location())(1)-
						  cell->center()(1), s);
				}
...
//				double	thetta = 1.0 / (pow(d, 2)+0.001);//	eps=0.001
			...
			{	
				sum += j;
				for (int s = 0; s <= j; s++)
	 				b[sum + s] = pow(cell->vertex(vertex)[0]-cell->center()(0), j - s) *
					  pow(cell->vertex(vertex)[1]-cell->center()(1), s);
			}
			...
			sum_velocityX[cell->vertex_dof_index(vertex, 0)] += c*b;
			...
			sum_velocityY[cell->vertex_dof_index(vertex, 0)] += c*b;
